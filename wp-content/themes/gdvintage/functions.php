...
/******************************* Functions Of GoDaddy Base Theme Starts *******************************/
...
function cyberchimps_options_header_drag() {
	$options = array(
...
add_filter( 'header_drag_and_drop_options', 'cyberchimps_options_header_drag' );
function cyberchimps_default_header_drag() {
	$default = array(
					'cyberchimps_logo'  => __( 'Logo', 'cyberchimps' )
				);
	return $default;
}
add_filter( 'header_drag_and_drop_default', 'cyberchimps_default_header_drag' );

//social icon skin options
function cyberchimps_social_icon_options( $orig ) {
	$orig['vintage'] = get_template_directory_uri(). '/images/social/thumbs/icons-vintage.png';
	return $orig;
}
...
function cyberchimps_skin_color_options( $options ) {
	...
	$options = array(
		...
		'wood' => $imagepath . 'wood.png',
		'grunge' => $imagepath . 'grunge.png',
		'fabric' => $imagepath . 'fabric.png'
		);
	return $options;
}
...
/******************************* Functions Of GoDaddy Base Theme Ends *******************************/
 
/******************************* More Functions for Vintage Theme Starts *******************************/
 
add_action( 'init', 'initial_child_function' );
function initial_child_function() {
	remove_action( 'init', 'default_display');
}	

function cyberchimps_post_comments_child() {
	global $post;
	...
	if( $show ):
		if ( ! post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) : ?>
			<span class="comments-link"><?php comments_popup_link( __( '0 Comment', 'cyberchimps' ), __( '1 Comment', 'cyberchimps' ), __( '% Comments', 'cyberchimps' ) ); ?></span>
    <?php endif;
	endif;
}
...
echo get_the_date('y/m/d').' at '.get_the_time('g.ia').' &nbsp;  /  &nbsp; by <a href="'.get_author_posts_url(get_the_author_meta( 'ID' )).'">'.get_the_author().'</a> &nbsp;  /  &nbsp; <a href="'.get_permalink().'">'.get_comments_number().' '.$get_comments_number.'</a>';
}
function cyberchimps_featured_image_child() {
	global $post;
...
//remove fields
function cyberchimps_remove_vintage_options( $original ) {
	$new = cyberchimps_remove_options( $original, array( 'searchbar', 'callout_title' ) );
	return $new;
}
add_filter( 'cyberchimps_field_filter', 'cyberchimps_remove_vintage_options', 99 );

//add social icons to menu
function cyberchimps_add_social_menu( $menu ) {
	if( function_exists( 'cyberchimps_get_header_social_icons' ) ) {
		$social = '<li id="social">' . cyberchimps_get_header_social_icons() . '</li>';
		$new_menu = $menu . $social;
		return $new_menu;
	}
	else {
		return $menu;
...
add_filter( 'wp_nav_menu_items', 'cyberchimps_add_social_menu', 10, 5 );
add_filter( 'fallback_nav_menu_items', 'cyberchimps_add_social_menu', 10, 5 );

/* remove fields */
function cyberchimps_remove_fields( $orig ) {
	$new_fields = cyberchimps_remove_options( $orig, array( 'contact_details' ) );
	return $new_fields;
}
add_filter( 'cyberchimps_field_filter', 'cyberchimps_remove_fields' );
// Remove conatct section.
function cyberchimps_contact_section( $original ) {
	$new = cyberchimps_remove_options( $original, array( 'cyberchimps_header_details_section' ) );
	return $new;
}
add_filter( 'cyberchimps_sections_filter', 'cyberchimps_contact_section' );
/******************************* More Functions for Vintage Theme Ends *******************************/
?>